...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (firstname == "" || lastname == "" || password == "" || email == "") {
      res.status(400).json({ msg: "All Fields are required", status: 400 });
    }
    if (existinguser) {
      ...
      if (user) {
        res.status(201).json({
          success: "User Created, You can login in to your accout to continue",
        });
      } else {
  ...
  try {
    ...
    if (emailUser) {
      ...
      if (pwd) {
        ...
        res.status(403).json({ error: "Passwod not correct" });
      }
    } else {
...
const updateUser = async (req, res) => {
  ...
  try {
    if (!firstname || !lastname || !username || !email) {
      ...
      const user = await User.findById(user_id);
      if (user) {
        const newUser = await User.findOneAndUpdate(user._id, req.body, {
          new: true,
        });
        if (newUser) {
          res
            .status(200)
            .json({ success: "User Updated successfully", newUser });
        } else {
